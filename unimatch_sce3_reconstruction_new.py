...
def bidsa_loss(f_s1, f_s2, f_d1, f_d2):
    s_loss = torch.norm(f_s1 - f_s2, p=2)  
    d_loss = torch.norm(f_d1 - f_d2, p=2) 

 
    s_loss = s_loss / torch.norm(f_s1, p=2)  
...
def main():
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x),
            ...
            # loss_bidsa = bidsa_loss(f_s_injected_s1, f_s_injected_s2, f_d_reinforced_s1 , f_d_reinforced_s2)
            # loss_recon = criterion_recon(recon_output, img_u_w)
            # loss_recon = 0
            loss = (loss_x + loss_u_s1 * 0.25 + loss_u_s2 * 0.25 + loss_u_w_fp * 0.25 ) / 1.75
            ...
            # total_loss_bidsa.update(loss_bidsa.item())
            ...
            if rank == 0:
                ...
                writer.add_scalar('train/loss_w_rec', loss_rec.item(), iters)
                writer.add_scalar('train/mask_ratio', mask_ratio, iters)
            
            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                logger.info('Iters: {:}, Total loss: {:.3f}, Loss x: {:.3f}, Loss s: {:.3f}, Loss w_fp: {:.3f}, loss_rec: {:.3f}, Mask ratio: '
                            '{:.3f}'.format(i, total_loss.avg, total_loss_x.avg, total_loss_s.avg, 
                                            total_loss_w_fp.avg, total_loss_rec.avg, total_mask_ratio.avg))
