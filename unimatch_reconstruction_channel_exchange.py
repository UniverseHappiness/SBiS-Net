...
from model.unet import UNet_unimatch_reconstruction
...
# 
# [0.8904299  0.81201176 1.43829132 0.47129227]
# [0.89682986 0.82194235 1.48522769 0.40584383]
# [0.88006056 0.79201056 2.53351806 0.6967521 ]
# [0.88956561 0.80738134 1.38011154 0.37265482]
def main():
    ...
    model = UNet_unimatch_reconstruction(in_chns=1, class_num=cfg['nclass'])    
    if rank == 0:
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x),
            ...
            preds, preds_fp = model(torch.cat((img_x, img_u_w)), need_fp=True, channel_exchange=True)
            pred_x, pred_u_w = preds.split([num_lb, num_ulb])
            pred_u_w_fp = preds_fp[num_lb:]

            pred_u_s1, pred_u_s2 = model(torch.cat((img_u_s1, img_u_s2)), channel_exchange=True).chunk(2)
            ...
            # loss_recon = 0
            loss = (loss_x + loss_u_s1 * 0.25 + loss_u_s2 * 0.25 + loss_u_w_fp * 0.25 + loss_recon * 0.25) / 2
            ...
            total_loss_w_re.update(loss_recon.item())
            ...
            if rank == 0:
                ...
                writer.add_scalar('train/loss_recon', loss_recon.item(), iters)
                writer.add_scalar('train/mask_ratio', mask_ratio, iters)
            
            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                logger.info('Iters: {:}, Total loss: {:.3f}, Loss x: {:.3f}, Loss s: {:.3f}, Loss w_fp: {:.3f}, Loss recon: {:.3f}, Mask ratio: '
                            '{:.3f}'.format(i, total_loss.avg, total_loss_x.avg, total_loss_s.avg, 
                                            total_loss_w_fp.avg,total_loss_w_re.avg, total_mask_ratio.avg))
