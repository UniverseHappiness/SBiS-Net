...
# momenten 0.999
# def bidsa_loss(f_s1, f_s2, f_d1, f_d2):
#     # 计算浅层特征一致性损失 + 深层特征一致性损失
#     s_loss = torch.norm(f_s1 - f_s2, p=2)  # 浅层特征一致性
#     d_loss = torch.norm(f_d1 - f_d2, p=2)  # 深层特征一致性

#     # 归一化损失
#     s_loss = s_loss / torch.norm(f_s1, p=2)  # 标准化为相对损失
#     d_loss = d_loss / torch.norm(f_d1, p=2)

#     consistency_loss = (s_loss + d_loss) / 2
#     return consistency_loss


def threshold_top_percentile(B, p):
    """Get binary mask for top-p percentile values"""
    B_flat = B.view(B.size(0), -1) # torch.Size([12, 65536])
    k = int(B_flat.size(1) * p) # 前1%元素
    thresholds = torch.kthvalue(B_flat, B_flat.size(1) - k, dim=1).values # torch.Size([12])
    R = (B >= thresholds.view(-1, 1, 1, 1)).float()
    return R

def compute_boundary_map(P):
...
def bidsa_loss(f_s1, f_s2, f_d1, f_d2):
    ...
    Args:
        ...
        f_d2: 深层特征2 [B, C, H, W]
    """
    # 将特征从 [B, C, H, W] 转换为 [B, C, H*W]
    f_s1 = f_s1.view(f_s1.size(0), f_s1.size(1), -1)
    f_s2 = f_s2.view(f_s2.size(0), f_s2.size(1), -1)
    f_d1 = f_d1.view(f_d1.size(0), f_d1.size(1), -1)
    f_d2 = f_d2.view(f_d2.size(0), f_d2.size(1), -1)

    # 计算空间位置对齐的余弦相似度（按通道维度计算）
    s_sim = F.cosine_similarity(f_s1, f_s2, dim=1)  # [B, H*W]
    d_sim = F.cosine_similarity(f_d1, f_d2, dim=1)  # [B, H*W]

    # 对每个样本的所有空间位置取平均
    s_loss = 1 - s_sim.mean(dim=1)  # [B]
    d_loss = 1 - d_sim.mean(dim=1)  # [B]

    # 对批次中的样本取平均
    consistency_loss = (s_loss.mean() + d_loss.mean()) / 2
    return consistency_loss
...
def main():
    ...
    model = BiDSA_UNet(in_chns=1, class_num=cfg['nclass'])    
    if rank == 0:
    ...
    if os.path.exists(os.path.join(args.save_path, 'latest.pth')):
        ...
        previous_best_test = checkpoint['previous_best_test']
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        total_loss_bidsa = AverageMeter()
        total_loss_rec =  AverageMeter()
        ...
        for i, ((img_x, mask_x),
            ...
            model.train()
            # recon_output = model(img_u_w, need_reconstruction=True, need_fp=True)
            ...
            pred_u_s, pred_u_s_fp = model(torch.cat((img_u_s1, img_u_s2)), need_fp_sce=True)
            pred_u_s1, pred_u_s2 = pred_u_s_fp.chunk(2)

            
            rec_img = model(img_u_w, need_rec=True)
            B = compute_boundary_map(pred_u_s1.detach())
            M_R = threshold_top_percentile(B, p=0.01) 
            
            f_s_injected_s1, f_d_reinforced_s1 = model(img_u_s1, need_bidsa=True)
            f_s_injected_s2, f_d_reinforced_s2 = model(img_u_s2, need_bidsa=True)
            ...
            loss_rec = masked_l1_loss(rec_img, img_u_w, M_R) * 0.25
            ...
            loss_bidsa = bidsa_loss(f_s_injected_s1, f_s_injected_s2, f_d_reinforced_s1 , f_d_reinforced_s2) * 0.25
            # loss_recon = criterion_recon(recon_output, img_u_w)
            # loss_recon = 0
            loss = (loss_x + loss_u_s1 * 0.25 + loss_u_s2 * 0.25 + loss_u_w_fp * 0.25) / 2
            ...
            total_loss_rec.update(loss_rec.item()*4)
            total_loss_bidsa.update(loss_bidsa.item()*4)
            ...
            if rank == 0:
                ...
                writer.add_scalar('train/loss_bidsa', loss_bidsa.item(), iters)
                writer.add_scalar('train/loss_rec', loss_rec.item(), iters)
                writer.add_scalar('train/mask_ratio', mask_ratio, iters)
            
            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                logger.info('Iters: {:}, Total loss: {:.3f}, Loss x: {:.3f}, Loss s: {:.3f}, Loss w_fp: {:.3f}, loss_rec: {:.3f}, loss_bidsa: {:.3f}, Mask ratio: '
                            '{:.3f}'.format(i, total_loss.avg, total_loss_x.avg, total_loss_s.avg, 
                                            total_loss_w_fp.avg, total_loss_rec.avg, total_loss_bidsa.avg, total_mask_ratio.avg))
        ...
        is_best_test = mean_dice_t > previous_best_test
        previous_best = max(mean_dice, previous_best)
        previous_best_test =  max(mean_dice_t, previous_best_test) 
        if epoch >= 15:
            torch.save(checkpoint, os.path.join(args.save_path, "epoch" + str(epoch) + '.pth'))
        if rank == 0:
            checkpoint = {
            ...
            if is_best_test:
                torch.save(checkpoint, os.path.join(args.save_path, 'best_test.pth'))
