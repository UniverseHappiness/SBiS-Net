...
def main():
    ...
    trainloader_u_mix = DataLoader(trainset_u, batch_size=cfg['batch_size'],
                                   pin_memory=True, num_workers=4, drop_last=True, sampler=trainsampler_u_mix)
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        total_loss_w_elastic_0_1 = AverageMeter()
        total_loss_w_elastic_1_2 = AverageMeter()
        total_loss_w_elastic_2_3 = AverageMeter()
        total_loss_w_elastic_3_4 = AverageMeter()
        ...
        for i, ((img_x, mask_x),
            ...
            low_feature_w, high_feature_w = model(img_u_w, contrastive_feature=True)
            ...
            loss = (loss_x + loss_u_s1 * 0.25 + loss_u_s2 * 0.25 + loss_u_w_fp * 0.5 + loss_elastic_0_1*0.25 + loss_elastic_1_2*0.25 + loss_elastic_2_3*0.25 + loss_elastic_3_4*0.25 ) / 3
            ...
            total_loss_w_elastic_0_1.update(loss_elastic_0_1.item())
            total_loss_w_elastic_1_2.update(loss_elastic_1_2.item())
            total_loss_w_elastic_2_3.update(loss_elastic_2_3.item())
            total_loss_w_elastic_3_4.update(loss_elastic_3_4.item())
            ...
            if rank == 0:
                ...
                writer.add_scalar('train/loss_elastic_0_1',loss_elastic_0_1.item(),iters)
                writer.add_scalar('train/loss_elastic_1_2',loss_elastic_1_2.item(),iters)
                writer.add_scalar('train/loss_elastic_2_3',loss_elastic_2_3.item(),iters)
                writer.add_scalar('train/loss_elastic_3_4',loss_elastic_3_4.item(),iters)
                writer.add_scalar('train/mask_ratio', mask_ratio, iters)
            
            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                logger.info('Iters: {:}, Total loss: {:.3f}, Loss x: {:.3f}, Loss s: {:.3f}, Loss w_fp: {:.3f}, Loss_w_elastic_0_1: {:.3f}, Loss_w_elastic_1_2: {:.3f}, Loss_w_elastic_2_3: {:.3f}, Loss_w_elastic_3_4: {:.3f}, Mask ratio: '
                            '{:.3f}'.format(i, total_loss.avg, total_loss_x.avg, total_loss_s.avg, 
                                            total_loss_w_fp.avg, total_loss_w_elastic_0_1.avg, total_loss_w_elastic_1_2.avg, total_loss_w_elastic_2_3.avg, total_loss_w_elastic_3_4.avg,total_mask_ratio.avg))
        model.eval()
        dice_class = [0] * 3
        dice_class_t = [0] * 3
        with torch.no_grad():
